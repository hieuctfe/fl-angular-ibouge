...
module.exports = {
	...
	getHistory: function(room, limit, offset) {
		return new Promise((resolve, reject) => {
			...
			Chat.findOne({'room': room}, {messages: {$slice: [offset, limit]}}, 
			function(err, chat) {
				if (err) {
	...
	updateLastMessage: function(room) {
		return new Promise((resolve, reject) => {
			...
			Chat.findOneAndUpdate({'room': room}, {'$set': {"messages.0.read_at": Date.now()}}, 
			function(err, chat) {
				if (err) {
					return reject({status: 500, message: 'Internal server error'});
				}
	...
	createChat: function(myId, room, users, isGroupChat, groupChatName) {
		return new Promise((resolve, reject) => {
			...
			function(err, chat) {
				...
				User.find({'_id': {$in: users}}, ['fname', 'lname', 'profile_pic'],
	...
	getGroupChatsByCreatedBy: function(userId, fields) {
		return new Promise((resolve, reject) => {
			...
			Chat.find({'created_by': userId, 'is_group_chat': true}, fields,
			function(err, chats) {
				if (err || !chats) {
					return reject({status: 500, message: 'Internal server error'});
				}
